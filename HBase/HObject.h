...
#define NOCOLOR ((*iscolorful)?std::string("\033[1m").c_str():std::string("").c_str())
...
protected: \
QMap<QString,HPointer(_name::*)(HArgs args)> memberfuncs##_name; \
public: HPointer exec(QString __name,HArgs args){ \
...
class HObject;
typedef QSharedPointer<HObject> HPointer;
typedef QWeakPointer<HObject> HWeakPointer;
typedef QVector<HPointer> HArgs;
class HObject
{
public:
	HObject() {
...
public:
	...
	HError(HError::ELEVEL Elevel = NONE, QString Why = "", HPointer _ret = nullptr) {
		this->elevel = Elevel;
		this->why = Why;
		this->ret = _ret;
	};
